...
const Profile = () => {
    ...
    const getUserData = async () => {
        ...
        const result = await axios.get('http://localhost:4200/api/users')
        console.log(result);
    ...
    const submit = async event => {
        ...
        const result = await axios.post('http://localhost:4200/api/update_avatar', data)
        console.log(result)
        getUserData();
        setOpen(!open)
    }
    ...
    const getMyPlaylists = async () => {
        var resp2 = await axios.get("/api/user_playlists/");
